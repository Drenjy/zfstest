...
#
# ident	"@(#)ce_number_of_processors.ksh	1.5	07/11/01 SMI"
#

#
# Function ce_number_of_processors
#
# Check:  the number of processors on the system
#
# Args:
#	$1: minimum=x (the minimum number of processors required)
#		if no minimum then set value to 0
#	$2: maximum=y (the maximumnumber of processors required)
...
function ce_number_of_processors
{
	...
	# otherwise.
	#
	errmsg="ce_number_of_processors expects 2 arguments (minimum=X and maximum=Y)"
	[[ $# -ne 2 ]] && abort $errmsg
	...
	# if minimum = maximum than that's the number of processors
	...
	if [[ $minimum = $maximum ]] 
	then
		req_label_1="Number of processors required"
		req_value_1=$minimum
	...
	# there are 2 checks here
		...
		num_checks=2
		req_label_1="Minimum number of processors"
		req_value_1=$minimum
		req_label_2="Maximum number of processors"
		req_value_2=$maximum
	fi

        # If operation is to dump info, dump and return
	[[ $OPERATION = "dump" ]] && {
		ce_number_of_processors_doc
		return $PASS
	}
	...
	[[ $OPERATION = "verify" ]] && {
		...
		num_procs=$(psrinfo | grep on-line | wc -l)

		# Check 1

		if [[ $minimum -eq 0 || $num_procs -ge $minimum ]] 
		then
			result_1=PASS
		else
			result_1=FAIL
			result_msg_1="system has $num_procs on-line processors (minimum of $minimum needed)"
		fi

		# Check 2

		[[ $num_checks -eq 2 ]] && {
			if [[ $maximum -eq 0 || $num_procs -le $maximum ]] 
			then
				result_2=PASS
			else
				result_2=FAIL
				result_msg_2="system has $num_procs on-line processors (maximum should be $maximum)"
			fi
		}
	}
...
function ce_number_of_processors_doc 
{
	cat >&1 << EOF

Check ce_number_of_processors $(TASK)
=========================================
Description:
	Check if this system has a certain number of processors (either a
	...
	maximum.  Otherwise one check is done - the number of processors
	required
Arguments:
	#1 - minimum=<the minimum number of processors>, 0 if no min
	#2 - maximum=<the maximum number of processors>, 0 if no max
