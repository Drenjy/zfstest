...
#
# ident	"@(#)stf_configlookuptool.nawk	1.5	07/04/12 SMI"
#
...
function printcolumn() {
	gsub("{na}", "");
	if (tolower(outcolumn) ~ "flags") {
		$1 = $2 = $3 = $4 = "";
		print $0;
	} else if (tolower(outcolumn) ~ "path") {
		print $4;
	} else if (tolower(outcolumn) ~ "alias") {
		print $3;
	} else {
		exit(1);
	}
...
#
	...
	tool=		ARGV[3];	delete ARGV[3]; # Tool name.
	outcolumn=	ARGV[4];	delete ARGV[4];	# Column to print.
